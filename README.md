...
- **録音キャンセル**: 録音中に Esc キーで即時キャンセル（認識処理・WAV ファイルを破棄）
- **Claude 後処理**: 認識結果を Claude でフィラー除去・句読点補完（設定で ON/OFF、デフォルト OFF）
- **WAV 自動削除**: 認識完了後に録音ファイルを自動削除（設定で変更可、デフォルト ON）
...
録音中に **Esc** を押すとキャンセルされ、テキスト入力は行われません。
...
│   │   └── TextPostProcessor.swift      # Claude による後処理（フィラー除去・句読点補完）
│   ├── Views/
